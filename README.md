...
<body class="bg-slate-100 text-slate-800 min-h-screen flex flex-col md:flex-row pb-16 md:pb-0">
        ...
        <div>
            <div class="flex items-center gap-3 mb-6">
                ...
                <div>
                    ...
                    <span class="text-xs text-indigo-300">Smart Attendance v11.0</span>
                </div>
            </div>
                ...
                <div class="lg:col-span-2 bg-white p-5 md:p-6 rounded-2xl shadow-sm border border-slate-200">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            ...
                            <span class="text-[11px] text-slate-400 mt-1 block">เช็กชื่อก่อนเวลานี้ = มาตรงเวลา</span>
                        </div>
                        <div class="bg-slate-50 p-4 rounded-2xl border border-slate-200">
                            ...
                            <span class="text-[11px] text-slate-400 mt-1 block">เช็กชื่อหลังเวลานี้ = สาย / เกินกำหนด</span>
                        </div>
                    </div>

                    <div class="p-4 bg-indigo-50 text-indigo-900 rounded-2xl border border-indigo-100 font-medium text-xs space-y-1 mb-4">
                        <p class="font-bold text-sm text-indigo-800">📌 กฎการกำกับเวลาอัตโนมัติ:</p>
                        <p>• เช็กชื่อตั้งแต่เปิดคาบ ถึง <b id="textOnTimeShow">08:30</b> ➔ สถานะ: <span class="bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded font-bold">มาตรงเวลา</span></p>
                        <p>• เช็กชื่อหลัง <b id="textOnTimeShow2">08:30</b> ถึง <b id="textLateShow">09:00</b> ➔ สถานะ: <span class="bg-amber-100 text-amber-700 px-2 py-0.5 rounded font-bold">เข้าเรียนสาย</span></p>
                        <p>• เช็กชื่อหลัง <b id="textLateShow2">09:00</b> เป็นต้นไป ➔ สถานะ: <span class="bg-rose-100 text-rose-700 px-2 py-0.5 rounded font-bold">สายมาก / ขาด</span></p>
                    </div>

                    <!-- นำเข้า Sync Code -->
                    <div class="pt-4 border-t border-slate-100">
                        <label class="block text-xs font-bold text-slate-700 mb-1">📥 นำเข้าผลการเช็กชื่อจากเครื่องนักเรียน (ถ้าแยกเครื่องใช้งาน):</label>
                        <div class="flex gap-2">
                            <input type="text" id="pasteLogInput" placeholder="วางโค้ดบันทึกจากเครื่องนักเรียน..." class="flex-1 border border-slate-300 rounded-xl px-3 py-2 text-xs outline-none focus:ring-2 focus:ring-indigo-500">
                            <button onclick="importStudentLogData()" class="bg-indigo-600 hover:bg-indigo-700 text-white px-4 py-2 rounded-xl text-xs font-bold shadow">
                                นำเข้าตาราง
                            </button>
                        </div>
                    </div>
    ...
    <script>
        ...
        let db = JSON.parse(localStorage.getItem('smart_attendance_db_v11')) || {
            timeRules: {
                onTimeLimit: '08:30',
                lateLimit: '09:00'
            },
        ...
        function saveData() {
            localStorage.setItem('smart_attendance_db_v11', JSON.stringify(db));
        }
        ...
        function saveTimeRules() {
            ...
            document.getElementById('textOnTimeShow').innerText = onTimeVal;
            document.getElementById('textOnTimeShow2').innerText = onTimeVal;
            document.getElementById('textLateShow').innerText = lateVal;
            document.getElementById('textLateShow2').innerText = lateVal;
        }

        // --- ระบบเช็กชื่อคำนวณเวลาอัตโนมัติ (มาตรง / สาย / สายมาก) ---
        function confirmStudentSelfScan() {
            try {
                ...
                // ตรวจสอบว่าเคยลงชื่อในวันเดียวกันหรือยัง
                const alreadyChecked = db.attendanceLogs[currentClassroom].some(l => l.id === studentId && l.date === dateStr);
                if (alreadyChecked) {
                    alert(`⚠️ ${studentName} ได้ลงชื่อเข้าเรียนของวันนี้ไปแล้วครับ`);
                    return;
                }
        ...
        function importStudentLogData() {
            ...
            if (!code) {
                alert("กรุณาวางโค้ดบันทึกจากเครื่องนักเรียน");
                return;
            }

            try {
                const log = JSON.parse(atob(code));
                if (!db.attendanceLogs[currentClassroom]) db.attendanceLogs[currentClassroom] = [];
                db.attendanceLogs[currentClassroom].unshift(log);
                saveData();
                renderAttendanceLogs();
                document.getElementById('pasteLogInput').value = '';
                alert(`✅ ดึงข้อมูลเช็กชื่อของ ${log.name} เข้าสู่ตารางเรียบร้อยแล้ว!`);
            } catch (e) {
                alert("❌ รหัสข้อมูลไม่ถูกต้อง");
            }
        }

        // --- จัดการห้องเรียน & นักเรียน (CRUD) ---
